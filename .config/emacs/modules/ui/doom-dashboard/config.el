...
(defvar +doom-dashboard-functions
  '(doom-dashboard-widget-banner
    doom-dashboard-widget-banner-title
...
(defvar +doom-dashboard-banner-file "marivector.png"
...
(defface doom-dashboard-footer-icon '((t (:inherit nerd-icons-pink)))
...
(defface custom-command-key-face '((t (:foreground "#faa7d7" :weight bold)))
  "Dashboard Key shortcut color."
  :group 'doom-dashboard)
(defface custom-footer-icon-color '((t(:foreground "faa7d7")))
"Dashboard footer icon color"
:group 'doom-dashboard)
;;
;;; Major mode

(define-derived-mode +doom-dashboard-mode special-mode
  (format "T O U H O U M A C S v%s" doom-version)
    ...
    (insert
     ...
     (propertize
      (+doom-dashboard--center
       +doom-dashboard--width
       (format "window total width: %d" (window-total-width)))
...
(defun doom-dashboard-widget-footer ()
  (insert
   ...
   (+doom-dashboard--center
    ...
    (with-temp-buffer
      (insert-text-button (or (nerd-icons-codicon "nf-cod-octoface" :face 'custom-footer-icon-color :Height 2 :V-adjust 0)
                              (propertize "github" 'face 'doom-dashboard-footer))
                          'action (lambda (_) (browse-url "https://github.com/JewelDesu"))
                          'follow-link t
                          'help-echo "Open github")
      (buffer-string)))
   "\n"))

(defun doom-dashboard-widget-banner-title()
  (insert
   (+doom-dashboard--center
    (- +doom-dashboard--width 5)
    (with-temp-buffer
    (propertize "M A G I C A L   🖤   E M A C S" 'font-lock-face '(:foreground "violet")
              )))"\n"))
